...
class DatabricksConfigParser(fileLocation: Option[String])(implicit logger: Logger) {
  ...
  private lazy val configFileContent = Source.fromFile(fileLocation.getOrElse(defaultConfigFilePath)).getLines.toSeq
  ...
  private def parseConfigFile(content: Seq[String]): Map[String,Map[String, String]] = {
    def parseRecurse(result: Map[String,Map[String, String]], content: Seq[String]): Map[String, Map[String, String]] = {
      content match {
        case head +: tail => {
          head.startsWith("[") match {
            case true => {
              ...
              val mapSettings: Map[String, String] = settings.map(_.split("=")).map(a => (a(0).trim(), a(1).trim())).toMap
